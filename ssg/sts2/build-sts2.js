...
async function buildEncounters(encounters, runStats, sitemap) {
    ...
    const encounterLinks = encounters.map(e => {
        ...
        const killDisplay = stats.kills > 0 ? `<div style="color: var(--red); font-size: 1.5rem; font-weight: bold; white-space: nowrap;">${stats.kills} Kills</div>` : '';
        ...
        <a href="/encounters/${slug}/" class="card-item" aria-label="${e.name}: encounters ${stats.encountered}">
            ...
            <div class="card-stats" style="flex-shrink: 0;">
                ${killDisplay}
            </div>
            ${winBar}
        </a>`;
