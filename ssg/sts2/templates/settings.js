...
export function settingsTemplate() {
            ...
            <div style="margin-top: 40px; border-top: 1px solid #333; padding-top: 30px;">
                ...
                <section style="margin-top: 30px;">
                    ...
                    <div style="margin-top: 15px;">
                        <p class="text-muted" style="font-size: 0.85rem;">Supa-Pro tip: Paste your directory below to get there quicker in the future.</p>
    ...
    document.addEventListener("DOMContentLoaded", function() {
        ...
        supabase.auth.onAuthStateChange(async (event, session) => {
            ...
            if (user && user.id !== currentUserId) {
                ...
                setTimeout(async () => {
                    ...
                    const { data: s2sUser } = await supabase.from('s2s_users').select('save_dir').eq('supabase_user_id', user.id).maybeSingle();
                    console.log("Settings: Profile fetch result ->", !!profile);
                    if (profile) {
                        usernameInput.value = profile.username;
                        slugPreview.textContent = profile.slug;
                        originalSlug = profile.slug;
                        if (s2sUser?.save_dir) saveDirInput.value = s2sUser.save_dir;
        ...
        saveBtn.onclick = async () => {
            ...
            const newSaveDir = saveDirInput.value.trim();
            if (!newName) return;
